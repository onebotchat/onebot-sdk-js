...
import { EventHandler, OneBotEventType, OneBotEvent } from './events';
import {
  Account,
  Contact,
  Group,
...
export class OneBotClient {
  ...
  public constructor(token: string, options?: OneBotClientOptions) {
    this.token = token;
    this.uuid = userFromToken(token);
    this.logLevel = options?.logLevel ?? LogLevel.INFO;
    this.logger = makeConsoleLogger('onebot-js');
    let baseUrl = options?.baseUrl ?? 'https://api.onebot.chat/client/v1';
    if (baseUrl.endsWith('/')) {
    ...
    this.uploadPlugin = options?.upload ?? new OneBotUploadPlugin(this.uuid, this.baseUrl, this.token);
  }
  ...
  public async authorize(provider: string, strategy?: string | AuthorizeCallback, cb?: AuthorizeCallback): Promise<string | undefined> {
    ...
    const win = popup(url, 'onebot-authorize-window');
    if (!win) {
   ...
   * @returns
   */
  public on(type: OneBotEventType, handler: EventHandler) {
   ...
   * @param handler
   */
  public off(type: OneBotEventType, handler: EventHandler) {
  ...
  getUserAgent() {
    return this.userAgent || `onebot-javascript-client-browser-${process.env.PKG_VERSION}`;
  }
